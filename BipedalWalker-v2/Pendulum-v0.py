...
STEP = 2000				# step limit in an episode
...
class DQN():
	...
	def create_training_method(self):
		...
		self.Q_action = tf.reduce_sum(tf.mul(self.Q_value, self.action_input), reduction_indices = 1)
		self.cost = tf.reduce_mean(tf.square(self.y_input - self.Q_action))
		# summary
		tf.scalar_summary("Q_action", self.Q_action)
